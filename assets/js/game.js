...
var Game = function() {
	...
	this.init = function(size, base, ui) {
		...
		this.caseHeight = base.width() / this.originalSize;
		// this.caseHeight = 70;
		this.level = [];
		this.typesOfGems = 5;
		this.fillEnd = true;
	...
	this.swipeGems = function(a, aID, b, bID) {
		...
		if (endStatus == false && this.switchEnd && a !== undefined && b !== undefined && aID >= 0 && bID >= 0 && aID <= this.size && bID <= this.size) {
			...
			b.attr('data-id', aID).animate({
				...
				that.switchEnd = true;
				var res = that.checkLines();

				if(res)
				{
					var temp = that.level[aID];
					that.level[aID] = that.level[bID];
					that.level[bID] = temp;
	...
	this.checkGemAround = function(gemType, position) {
		...
		var currentLine = parseInt(position / 8);

		var removeWArray = [];
		removeWArray.push(position);

		var l = -1;
		while (position + l != 0 && this.level[position + l] === gemType){
			removeWArray.push(position + l);
			l--;
		}

		var r = 1;
		while (position + r != 7 && this.level[position + r] === gemType){
			removeWArray.push(position + r);
            r++;
		}

		if(removeWArray.length >= 3 && position % this.lines)
		{
			this.removeClearedGemToLevel(removeWArray);
		}else{
			wflag = true;
		}
		...
		var removeHArray = [];
		removeHArray.push(position);

		var l = -1;
		while (position + l * 8 != 0 && this.level[position + l * 8] === gemType){
			removeHArray.push(position + l * 8);
			l--;
		}

		var r = 1;
		while (position + r * 8 != 64 && this.level[position + r * 8] === gemType){
			removeHArray.push(position + r * 8);
            r++;
		}

		if(removeHArray.length >= 3)
		{
			this.removeClearedGemToLevel(removeHArray);
		}else{
			hflag = true;
		}
...
function startSmallTimer()
{
	...
	perCount = setInterval(function () {
		console.log(i);
		i -= 1;
		if(i > 0)
		{
			$('#smallProgress').css('width', i + '%');
			document.getElementById("smallProgress").innerHTML = i + "%";
		}else if(i == 0){
			clearInterval(totalCount);
			clearInterval(perCount);

			$('#smallProgress').css('width', 0 + '%');
			document.getElementById("smallProgress").innerHTML = 0 + "s";

			var html = '<h3 style="margin-left:40%; height: 40px">Game Over</h3>';

			$("#totalProgress").empty();
			$("#totalProgress").append(html);
			endStatus = true;
		}
	}, 100);
}
...
function start()
{
	if(startStatus == false)
	{
		...
		totalCount = setInterval(function () {
			...
			if(i < 10)
			{
				document.getElementById("bigProgress").style.background = "#FF1493";
			}
			if (i > 0) {
				$('#bigProgress').css('width', width + '%');
				document.getElementById("bigProgress").innerHTML = i + "s";
			} else {
				clearInterval(totalCount);
				clearInterval(perCount);

				$('#bigProgress').css('width', 0 + '%');
				document.getElementById("bigProgress").innerHTML = 0 + "s";
